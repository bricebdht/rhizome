...
- Tailwind v4 wired through `@tailwindcss/vite` (no PostCSS config, no
  `tailwind.config.js` — v4 is configured from CSS).
- shadcn/ui initialized; `Button` and `Card` added as smoke test.
- Global CSS reset applied — Tailwind's preflight, included by
  `@import 'tailwindcss'`.
- The root `tsconfig.json` extends `tsconfig.base.json`, so tooling that
  reads it directly can find the `@/*` alias. shadcn's CLI refuses to
  initialize otherwise: it does not follow project references.
- `src/components/ui/**` exempted from `react-refresh/only-export-components`
  — shadcn ships its `cva` variants beside the component, and those files are
  regenerated by its CLI.

Scope out:

- any theming or dark mode toggle (Phase 5). The preset does define dark
  tokens; nothing switches to them.
- editing the generated primitives. They are treated as vendored.

Acceptance criteria:

- a demo page shows a shadcn `Button` and `Card` styled correctly.
- the emitted CSS contains the utilities used by that page and the resolved
  theme tokens — i.e. Tailwind scanned the sources and shadcn's variables are
  in scope.
